...
from docscorer.utils import join_utf_blocks
...
def score_numbers(word_chars: int, numbers: int) -> float:
    if word_chars <= 0:
        return 0
    return round((numbers / word_chars) * 100, 1)


def score_singular_chars(word_chars: int, singular_chars: int) -> float:
    if word_chars <= 0:
        return 0
    return round((singular_chars / word_chars) * 100, 1)


def score_punctuation(word_chars: int, punctuation_chars: int) -> float:
    if word_chars <= 0:
        return 0
            ...
            for document_file in file:
                ...
                condensed_data = [
                    (
                        ...
                        len(re.findall(punctuation_pattern, segment)),
                        len(re.findall(singular_chars_pattern, segment)),
                        len(re.findall(numbers_pattern, segment)),
                    )
                    for segment in document["text"].split("\n")
                ]
