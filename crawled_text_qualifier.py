...
import json
import sys
from utils.utils import average, precision_round
...
MENUS_AVERAGE_LENGTH = {lang: round(ref_punctuation * menu_length / val) for lang, val in LANGUAGES_PUNCTUATION.items()}
MENUS_AVERAGE_LENGTH["standard"] = average(MENUS_AVERAGE_LENGTH.values())
...
PUNCTUATION_PERCENT_MAX = {lang: round(val * punct_max / ref_punctuation,1) if val * punct_max / ref_punctuation < 100 else 100.0 for lang, val in LANGUAGES_PUNCTUATION.items()}
PUNCTUATION_PERCENT_MAX["standard"] = average(PUNCTUATION_PERCENT_MAX.values())

PUNCTUATION_PERCENT_BAD = {lang: (round(val * punct_bad[0] / ref_punctuation,1), round(val * punct_bad[1] / ref_punctuation, 1)) for lang, val in LANGUAGES_PUNCTUATION.items()}
PUNCTUATION_PERCENT_BAD["standard"] = (average([x[0] for x in PUNCTUATION_PERCENT_BAD.values()]), average([x[1] for x in PUNCTUATION_PERCENT_BAD.values()]))

PUNCTUATION_PERCENT_SEMIBAD = {lang: (round(val * punct_semibad[0] / ref_punctuation,1), round(val * punct_semibad[1] / ref_punctuation, 1)) for lang, val in LANGUAGES_PUNCTUATION.items()}
PUNCTUATION_PERCENT_SEMIBAD["standard"] = (average([x[0] for x in PUNCTUATION_PERCENT_SEMIBAD.values()]), average([x[1] for x in PUNCTUATION_PERCENT_SEMIBAD.values()]))

PUNCTUATION_PERCENT_DESIRED_MAX = {lang: round(val * punct_desired_max / ref_punctuation,1) for lang, val in LANGUAGES_PUNCTUATION.items()}
PUNCTUATION_PERCENT_DESIRED_MAX["standard"] = average(PUNCTUATION_PERCENT_DESIRED_MAX.values())

PUNCTUATION_PERCENT_DESIRED_MIN  = {lang: round(val * punct_desired_min / ref_punctuation,1) for lang, val in LANGUAGES_PUNCTUATION.items()}
PUNCTUATION_PERCENT_DESIRED_MIN["standard"] = average(PUNCTUATION_PERCENT_DESIRED_MIN.values())
...
BAD_CHARS_PERCENT_MAX = {lang: round(val * bad_chars_max / ref_bad_chars,1) if val * bad_chars_max / ref_bad_chars < 100 else 100.0 for lang, val in LANGUAGES_BAD_CHARS.items()}
BAD_CHARS_PERCENT_MAX["standard"] = average(BAD_CHARS_PERCENT_MAX.values())

BAD_CHARS_PERCENT_BAD = {lang: round(val * bad_chars_bad / ref_bad_chars,1) for lang, val in LANGUAGES_BAD_CHARS.items()}
BAD_CHARS_PERCENT_BAD["standard"] = average(BAD_CHARS_PERCENT_BAD.values())

BAD_CHARS_PERCENT_SEMIBAD = {lang: round(val * bad_chars_semibad / ref_bad_chars,1) for lang, val in LANGUAGES_BAD_CHARS.items()}
BAD_CHARS_PERCENT_SEMIBAD["standard"] = average(BAD_CHARS_PERCENT_SEMIBAD.values())

BAD_CHARS_PERCENT_DESIRED  = {lang: round(val * bad_chars_desired / ref_bad_chars,1) for lang, val in LANGUAGES_BAD_CHARS.items()}
BAD_CHARS_PERCENT_DESIRED["standard"] = average(BAD_CHARS_PERCENT_DESIRED.values())
...
NUMBERS_PERCENT_MAX = {lang: round(val * numbers_max / ref_numbers,1) if val * numbers_max / ref_numbers < 100 else 100.0 for lang, val in LANGUAGES_NUMBERS.items()}
NUMBERS_PERCENT_MAX["standard"] = average(NUMBERS_PERCENT_MAX.values())

NUMBERS_PERCENT_BAD = {lang: round(val * numbers_bad / ref_numbers,1) if val * numbers_max / ref_numbers < 100 else 100.0 for lang, val in LANGUAGES_NUMBERS.items()}
NUMBERS_PERCENT_BAD["standard"] = average(NUMBERS_PERCENT_BAD.values())

NUMBERS_PERCENT_SEMIBAD = {lang: round(val * numbers_semibad / ref_numbers,1) for lang, val in LANGUAGES_NUMBERS.items()}
NUMBERS_PERCENT_SEMIBAD["standard"] = average(NUMBERS_PERCENT_SEMIBAD.values())

NUMBERS_PERCENT_DESIRED  = {lang: round(val * numbers_desired / ref_numbers,1) for lang, val in LANGUAGES_NUMBERS.items()}
NUMBERS_PERCENT_DESIRED["standard"] = average(NUMBERS_PERCENT_DESIRED.values())
...
BIG_TEXT_MAX = {lang: round(ref_punctuation * big_text_max / val) for lang, val in LANGUAGES_PUNCTUATION.items()}
BIG_TEXT_MAX["standard"] = average(BIG_TEXT_MAX.values())

BIG_TEXT_MIN = {lang: round(ref_punctuation * big_text_min / val) for lang, val in LANGUAGES_PUNCTUATION.items()}
BIG_TEXT_MIN["standard"] = average(BIG_TEXT_MIN.values())
...
def valorate_punctuation(ref_language, punctuation_chars, word_chars):

    percent_max = PUNCTUATION_PERCENT_MAX[ref_language] if ref_language in PUNCTUATION_PERCENT_MAX else PUNCTUATION_PERCENT_MAX["standard"]
    percent_bad = PUNCTUATION_PERCENT_BAD[ref_language] if ref_language in PUNCTUATION_PERCENT_BAD else PUNCTUATION_PERCENT_BAD["standard"]
    percent_semibad = PUNCTUATION_PERCENT_SEMIBAD[ref_language] if ref_language in PUNCTUATION_PERCENT_SEMIBAD else PUNCTUATION_PERCENT_SEMIBAD["standard"]
...
def valorate_bad_chars(ref_language, bad_chars, word_chars):
    percent_max = BAD_CHARS_PERCENT_MAX[ref_language] if ref_language in BAD_CHARS_PERCENT_MAX else BAD_CHARS_PERCENT_MAX["standard"]
...
def valorate_numbers(ref_language, numbers, word_chars):
    percent_max = NUMBERS_PERCENT_MAX[ref_language] if ref_language in NUMBERS_PERCENT_MAX else NUMBERS_PERCENT_MAX["standard"]
    percent_bad = NUMBERS_PERCENT_BAD[ref_language] if ref_language in NUMBERS_PERCENT_BAD else NUMBERS_PERCENT_BAD["standard"]
...
def valorate_repeated(ref_language, document):
    menu_length = MENUS_AVERAGE_LENGTH[ref_language] if ref_language in MENUS_AVERAGE_LENGTH else MENUS_AVERAGE_LENGTH["standard"]
...
def valorate_text(ref_lang, lang_segments, scores_lang, document):
    ...
    language_score = valorate_lang(ref_lang, lang_segments, scores_lang, word_chars)
    punctuation_score = valorate_punctuation(ref_lang, sum(punctuation_chars), sum(word_chars))
    bad_chars_score = valorate_bad_chars(ref_lang, sum(bad_chars), sum(word_chars))
    numbers_score = valorate_numbers(ref_lang, sum(numbers), sum(word_chars))
    repeated_score = valorate_repeated(ref_lang, document)
    url_score = valorate_urls(ref_lang, document, word_chars)
    big_segments_scores = valorate_big_texts(ref_lang, lang_segments, word_chars)
